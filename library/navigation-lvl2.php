...
function coenv_base_intranet_submenu($postid) {
    $post = get_post($postid);
    $top_post = $post;
    // If the post has ancestors, get its ultimate parent and make that the top post
    if ($post->post_parent && $post->ancestors) {
    ...
    // Always start traversing from the top of the tree
    return coenv_base_intranet_submenu_get_children($top_post, $post);
}

function coenv_base_intranet_submenu_get_children($post, $current_page) {
    $menu = '';
    // Get all immediate children of this page
    $args = array(
        'depth' => 4,
