...
class coenv_base_date extends WP_Widget {
     ...
     function __construct() {
          parent::__construct(
               'coenv_base_date', // Base ID
               __('News date filter (COENV)', 'text_domain'), // Name
               array( 'description' => __( 'Allows filtering of news based on date', 'text_domain' ), ) // Args
          );
     }
     ...
     public function widget( $args, $instance ) {
          ...
          echo '<div data-url="/news-events/">';
          coenv_base_date_filter('post',$coenv_month,$coenv_year); // Date filter
          echo '</div>';
          echo $args['after_widget'];
     }
...
// register coenv_base_date widget
function register_coenv_base_date() {
    register_widget( 'coenv_base_date' );
}
add_action( 'widgets_init', 'register_coenv_base_date' );
